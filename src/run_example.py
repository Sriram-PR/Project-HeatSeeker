...
if __name__ == "__main__":
    print("=" * 60)
    print(" Running Tracking Pipeline Example ")
    print("=" * 60)
    ...
    has_valid_gt = data_loader.gt_corners is not None and any(
        gt is not None for gt in data_loader.gt_bboxes_with_id
    )
    if not has_valid_gt:
        print(
            f"Warning: No valid ground truth found for sequence {SEQUENCE_NAME}. Metrics cannot be calculated."
        )
    else:
    ...
    gif_output_path = (
        os.path.join(OUTPUT_DIR, f"{SEQUENCE_NAME}_example_{timestamp}.gif")
        if SAVE_GIF
        else None
    )
    csv_output_path = (
        os.path.join(OUTPUT_DIR, f"{SEQUENCE_NAME}_example_{timestamp}.csv")
        if SAVE_CSV
        else None
    )
    ...
    if has_valid_gt:
        if frame_update_data:
            ...
            for frame_data in frame_update_data:
                mot_accumulator_run.update(
                    *frame_data
                )  # Unpack (gt_ids, hyp_ids, dist_matrix)

            # --- Calculate and Display Metrics ---
            if not mot_accumulator_run.events.empty:
                print("Calculating MOT Metrics...")
                metrics_summary_df = calculate_mot_metrics(
                    mot_accumulator_run
                ...
                if not metrics_summary_df.empty:
                    print("\nSpecific Metrics:")
                    mota = (
                        metrics_summary_df["mota"].iloc[0]
                        if "mota" in metrics_summary_df.columns
                        else "N/A"
                    )
                    motp = (
                        metrics_summary_df["motp"].iloc[0]
                        if "motp" in metrics_summary_df.columns
                        else "N/A"
                    )
                    idf1 = (
                        metrics_summary_df["idf1"].iloc[0]
                        if "idf1" in metrics_summary_df.columns
                        else "N/A"
                    )
                    print(
                        f"  MOTA: {mota:.3f}"
                        if isinstance(mota, (float, int))
                        else f"  MOTA: {mota}"
                    )
                    print(
                        f"  MOTP: {motp:.3f}"
                        if isinstance(motp, (float, int))
                        else f"  MOTP: {motp}"
                    )
                    print(
                        f"  IDF1: {idf1:.3f}"
                        if isinstance(idf1, (float, int))
                        else f"  IDF1: {idf1}"
                    )
                else:
