...
try:
    ...
    print("Error: scikit-optimize not installed. Use: pip install scikit-optimize")
    exit()
...
param_space = [
    ...
    Categorical([5, 7, 9, 11], name="bilateral_d"),
    Real(20.0, 100.0, name="bilateral_sigmaColor"),
    Real(20.0, 100.0, name="bilateral_sigmaSpace"),
...
TRIALS_CSV_PATH = os.path.join(
    OUTPUT_DIR, f"{OPTIMIZATION_NAME}_trials_{TIMESTAMP}.csv"
)
VALIDATION_CSV_PATH = os.path.join(
    OUTPUT_DIR, f"{OPTIMIZATION_NAME}_validation_{TIMESTAMP}.csv"
)
...
def objective_function(**params):
    ...
    if tuple_param_name in current_params:
        ...
        try:
            ...
            if len(dims) == 2:
                current_params[tuple_param_name] = tuple(dims)
            else:
                raise ValueError("Invalid grid string format")
        except Exception as e:
            print(
                f"  ERROR: Invalid format for {tuple_param_name}: '{grid_str}'. Using (8, 8). Error: {e}"
            )
    ...
    try:
        ...
        print(f"  ERROR: Failed to create PipelineConfig: {e}")
        traceback.print_exc()
    ...
    for seq_name in TRAINING_SEQUENCES:
        ...
        try:
            ...
            if not data.frame_paths:
                error_msg = "No frames"
            elif data.gt_corners is None or not any(
                gt is not None for gt in data.gt_bboxes_with_id
            ):
                error_msg = "No valid GT"
            if error_msg:
                print(f"    Warning: {error_msg} for {seq_name}. Skipping.")
            else:
                pipeline = MotionDetectionPipeline(data, config)
                seq_frame_updates, _ = pipeline.run(
                    save_gif_path=None, save_csv_path=None
                )  # NO GIF/CSV during opt
                if seq_frame_updates:
                    for frame_data in seq_frame_updates:
                        combined_accumulator.update(*frame_data)
                    seq_success = True
                    total_frames_processed += len(seq_frame_updates)
                else:
                    error_msg = "Pipeline yielded no frame data"  # Can be verbose
        except Exception as e:
            error_msg = f"Pipeline Run ERROR: {e}"
            traceback.print_exc()
        if error_msg:
            sequences_with_errors.append(
                f"{seq_name}:{error_msg[:50]}"
            )  # Log truncated error
    ...
    score = worst_score
    metric_value = worst_metric_value
    ...
    if combined_accumulator.events.shape[0] > 0:
        try:
            ...
            summary = mh.compute(
                ...
                metrics=[OPTIMIZATION_TARGET_METRIC],
                name="opt_eval",
            )
            if not summary.empty and OPTIMIZATION_TARGET_METRIC in summary.columns:
                ...
                trial_log["error_metric"] = (
                    f"Metric '{OPTIMIZATION_TARGET_METRIC}' not found"
                )
        except Exception as e:
            print(f"  ERROR calculating combined metrics: {e}")
            traceback.print_exc()
            trial_log["error_metric"] = str(e)[:200]
            score = worst_score
    else:
        trial_log["error_metric"] = "No events accumulated"
        score = worst_score

    # --- Finalize Log ---
    trial_duration = time.time() - trial_start_time
    print(
        f"  Score: {score:.5f} ({OPTIMIZATION_TARGET_METRIC.upper()}: {metric_value:.5f}) (Time: {trial_duration:.2f}s)"
    )
    trial_log.update(
        {
            ...
            "processed_seq": ";".join(processed_sequences),
            "errors": "; ".join(sequences_with_errors),
    ...
    if optimization_call_count > 0 and optimization_call_count % 10 == 0:
        try:
            pd.DataFrame(optimization_trials_data).to_csv(
                TRIALS_CSV_PATH, index=False, float_format="%.5f"
            )
            print(f"  (Saved intermediate log to {TRIALS_CSV_PATH})")
        except Exception as e:
            print(f"  (Warning: Failed to save intermediate log: {e})")
    return score
...
if __name__ == "__main__":
    ...
    best_metric_value_actual = (
        (1.0 - best_score_minimized) if MAXIMIZE_METRIC else best_score_minimized
    )
    ...
    best_params_dict = {
        space.name: value for space, value in zip(param_space, best_params_list)
    }
    ...
    if tuple_param_name in best_params_dict:
        ...
        try:
            dims = list(map(int, grid_str.split("x")))
            if len(dims) == 2:
                best_params_dict[tuple_param_name] = tuple(dims)
        except Exception:
            best_params_dict[tuple_param_name] = (8, 8)  # Fallback
    ...
    if optimization_trials_data:
        try:
            ...
            print(f"ERROR saving final trials log: {e}")
    ...
    try:
        ...
        print(f"FATAL ERROR creating best config: {e}")
        exit()
    ...
    for seq_name in VALIDATION_SEQUENCES:
        ...
        for k, v in best_params_dict.items():
            seq_results[f"best_{k}"] = str(v) if isinstance(v, tuple) else v
        ...
        try:
            ...
            if not data_val.frame_paths:
                val_error_msg = "No frames"
                val_status = "Skipped"
            elif data_val.gt_corners is None or not any(
                gt is not None for gt in data_val.gt_bboxes_with_id
            ):
                val_error_msg = "No valid GT"
                val_status = "Skipped (No GT)"
            else:
                pipeline_val = MotionDetectionPipeline(data_val, best_config)
                csv_path_val = os.path.join(
                    OUTPUT_DIR, f"{seq_name}_VALIDATION_{TIMESTAMP}.csv"
                )
                print(f"    Running pipeline (saving CSV: {csv_path_val})...")
                val_frame_updates, _ = pipeline_val.run(
                    save_gif_path=None, save_csv_path=csv_path_val
                )

                if val_frame_updates:
                    val_acc = mm.MOTAccumulator(auto_id=True)
                    for frame_data in val_frame_updates:
                        val_acc.update(*frame_data)
                    if not val_acc.events.empty:
                        summary_val = calculate_mot_metrics(val_acc)  # Prints metrics
                        if not summary_val.empty:
                            seq_results.update(summary_val.iloc[0].to_dict())
                            val_status = "Success"
                        else:
                            val_error_msg = "Metric calc empty summary"
                            val_status = "Error"
                    else:
                        val_error_msg = "Accumulator empty"
                        val_status = "Error"
                else:
                    val_error_msg = "No tracking events"
                    val_status = "No Events"
        except Exception as e:
            print(f"    ERROR processing validation sequence {seq_name}: {e}")
            traceback.print_exc()
            val_error_msg = str(e)[:200]
            val_status = "Error"
        seq_results["status"] = val_status
        if val_error_msg:
            seq_results["error"] = val_error_msg
