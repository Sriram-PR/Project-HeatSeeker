...
def normalize_image(frame):
    ...
    if frame.dtype == np.uint16:
        return cv2_cpu.normalize(frame, None, 0, 255, cv2_cpu.NORM_MINMAX, dtype=cv2_cpu.CV_8U)
...
def check_gpu_mat(gpu_mat, expected_shape=None, expected_dtype=None, operation_name="Operation"):
    ...
    try:
        ...
        if expected_shape is not None and cpu_mat.shape != expected_shape:
            print(f"  [FAIL] {operation_name}: Shape mismatch. Exp {expected_shape}, Got {cpu_mat.shape}")
            return cpu_mat, False
        
        if expected_dtype is not None and cpu_mat.dtype != expected_dtype:
            print(f"  [FAIL] {operation_name}: Dtype mismatch. Exp {expected_dtype}, Got {cpu_mat.dtype}")
            return cpu_mat, False
...
if __name__ == "__main__":
    print("--- OpenCV CUDA Functionality & Timing Validation (Gaussian, CLAHE, MOG2, Farneback) ---")
    ...
    cpu_time = time_operation(cv2_cpu.GaussianBlur, [cpu_frames_gray_8u[0], gauss_ksize, gauss_sigmaX], 
                              TIMING_ITERATIONS, WARMUP_ITERATIONS)
    print(f"  Avg CPU Time: {cpu_time:.4f} ms")
    
    try:
        gpu_filter = cv2_gpu.cuda.createGaussianFilter(cv2_gpu.CV_8UC1, cv2_gpu.CV_8UC1, 
                                                       ksize=gauss_ksize, sigma1=gauss_sigmaX)
        ...
        for i in range(TIMING_ITERATIONS):
            gpu_frame_in.upload(cpu_frames_gray_8u[i % len(cpu_frames_gray_8u)], stream=stream)
            gpu_filter.apply(gpu_frame_in, gpu_frame_out, stream=stream)
            stream.waitForCompletion()
        ...
        _, success = check_gpu_mat(gpu_result, original_shape, np.uint8, "GPU Gaussian Result")
    ...
    try:
        cpu_mog2 = cv2_cpu.createBackgroundSubtractorMOG2(history=50, varThreshold=16, detectShadows=False)
        gpu_mog2 = cv2_gpu.cuda.createBackgroundSubtractorMOG2(history=50, varThreshold=16, detectShadows=False)
        gpu_fgMask = cv2_gpu.cuda_GpuMat(original_shape[0], original_shape[1], cv2_gpu.CV_8UC1)
        ...
        cpu_time = time_operation(cpu_mog2.apply, [cpu_frames_gray_8u[-1]], TIMING_ITERATIONS, 0)
        print(f"  Avg CPU Time: {cpu_time:.4f} ms")
    ...
    if NUM_TEST_FRAMES >= 2:
        ...
        cpu_time = time_operation(cv2_cpu.calcOpticalFlowFarneback, 
                                 [frame0_cpu, frame1_cpu, None, 0.5, 3, 15, 3, 5, 1.2, 0], 
                                 reduced_iterations, reduced_warmup)
        print(f"  Avg CPU Time: {cpu_time:.4f} ms")
        ...
        try:
            ...
            gpu_flow_out = cv2_gpu.cuda_GpuMat(original_shape[0], original_shape[1], cv2_gpu.CV_32FC2)
            
            try:
                flow_calculator = cv2_gpu.cuda.FarnebackOpticalFlow_create(
                    numLevels=3, pyrScale=0.5, winSize=15, numIters=3, polyN=5, polySigma=1.2, flags=0)
            ...
            _, success = check_gpu_mat(gpu_result, expected_flow_shape, np.float32, "GPU Farneback Result")
    ...
    if validation_passed:
        print("✓ All tested OpenCV CUDA functions executed successfully and passed verification.")
    else:
        print("✗ One or more OpenCV CUDA function tests failed execution or verification.")
