...
class DatasetLoader:
            ...
            with open(gt_path, 'r') as f:
                for line in f:
                    ...
                    if len(coords) == 8:
                        ...
                        print(f"Warning: Skipping line in {gt_path} with unexpected number of coordinates ({len(coords)}): {line.strip()}")
    ...
    @staticmethod
    def convert_corners_to_bbox(corners: list[float]) -> tuple[int, int, int, int] | None:
        ...
        try:
            ...
            if width == 0 and x_max != x_min: width = 1.0
            if height == 0 and y_max != y_min: height = 1.0
    ...
    def load_frame(self, idx: int) -> tuple[np.ndarray | None, int | None]:
        ...
        try:
            ...
            if frame is None:
                ...
                return None, None
            if frame.dtype == np.uint8: bit_depth = 8
            elif frame.dtype == np.uint16: bit_depth = 16
            else:
                print(f"Warning: Unexpected frame dtype {frame.dtype} for {frame_path}. Reloading as grayscale.")
