...
from preprocessing import normalize_image, preprocess
from visualization import draw_gt_on_frame, draw_tracks_on_frame
...
class MotionDetectionPipeline:
    ...
    def run(
        ...
        for idx in tqdm(
            range(num_frames),
            desc=f"Pipeline:{self.data.sequence_name}",
            unit="frame",
            leave=True,
        ):
            ...
            detections, det_boxes = self.detector.blobs(
                fgmask
            )  # Uses config.min_blob_area internally
            ...
            gt_ids = (
                [gt_info["id"]]
                if gt_info and "bbox" in gt_info and gt_info["bbox"]
                else []
            )
            gt_boxes = (
                [gt_info["bbox"]]
            ...
            dist_matrix = mm.distances.iou_matrix(
                gt_boxes, hyp_boxes, max_iou=1.0
            )  # Range [0, 1] is correct
        ...
        if save_gif_path and vis_frames:
            ...
            try:
                vis_frames_rgb = [
                    cv2.cvtColor(f, cv2.COLOR_BGR2RGB) for f in vis_frames
                ]
