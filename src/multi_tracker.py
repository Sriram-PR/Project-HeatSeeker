...
class MultiObjectTracker:
    ...
    def associate_and_track(self, detections: list[tuple[int, int]],
                             det_boxes: list[tuple[int, int, int, int]]) -> list[UKFTrack]:
        """
        ...
        """
        # 1. Predict
        trk_preds = np.array([trk.predict() for trk in self.tracks]) if self.tracks else np.zeros((0, 2))
        ...
        if len(self.tracks) > 0 and len(detections) > 0:
            cost_matrix = np.linalg.norm(trk_preds[:, None, :] - dets_np[None, :, :], axis=2)
            cost_matrix[cost_matrix > self.config.association_threshold] = 1e8
            try:
                trk_idx, det_idx = linear_sum_assignment(cost_matrix)
                valid_matches_mask = cost_matrix[trk_idx, det_idx] < self.config.association_threshold
                ...
                unmatched_tracks = [t for t in range(len(self.tracks)) if t not in matched_trk_indices]
                unmatched_detections = [d for d in range(len(detections)) if d not in matched_det_indices]
            except ValueError as e:
                print(f"Warning: linear_sum_assignment failed: {e}. Skipping association.")
        ...
        # 6. Prune Dead Tracks
        self.tracks = [trk for trk in self.tracks if trk.misses <= self.config.max_misses]
